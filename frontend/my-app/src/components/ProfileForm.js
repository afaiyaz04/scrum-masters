      ...
      <Form {...layout}>
        ...
        <Form.Item label="First Name:">
          <Input
            ...
            onChange={(e) => this.setState({ user: { ...this.state.user, nameFirst: e.target.value }})}
          />
        </Form.Item>
        <Form.Item label="Last Name:">
          <Input
            placeholder={this.state.user.nameLast}
            onChange={(e) => this.setState({ user: { ...this.state.user, nameLast: e.target.value }})}
          />
        </Form.Item>
        <Form.Item wrapperCol={{ span: 30 }} style={{ paddingTop: 50 }}>
            <Button type='primary' block onClick={() => this.updateHandler(this.state.user)} style={{ textAlign: 'center' }}>Update</Button>
        </Form.Item>
        <Form.Item wrapperCol={{ span: 30 }}>
            <Button type='primary' danger block onClick={() => {this.setState({ showDelete: true })}} style={{ textAlign: 'center' }}>Delete Account</Button>
            <Modal
              ...
              width={300}
              centered={true}>
                <h3 style={{ textAlign: 'center' }}>Are you sure? This action is irreversible.</h3>
                <Button type='primary' danger block onClick={() => this.deleteHandler()} style={{ textAlign: 'center' }}>Confirm</Button>
            </Modal>
        </Form.Item>
      </Form>
    );
